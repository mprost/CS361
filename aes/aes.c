...
// Allocates a matrix of Bytes, of a particular size
uint8_t** newMat(uint16_t num_rows, uint16_t num_cols) {
  uint8_t** mat = (uint8_t**)(malloc(sizeof(uint8_t**) * num_rows));
  for (int r = 0; r < num_rows; r++) {
    mat[r] = (uint8_t*)(malloc(sizeof(uint8_t) * num_cols));
  }
  return mat;
}

// Frees the pointers in a matrix
void freeMat(uint8_t** mat, uint16_t num_rows, uint16_t num_cols) {
  for (int r = 0; r < num_rows; r++) {
    free(mat[r]);
  }
  free(mat);
...
// Prints a formatted Byte value
void printXByte(uint8_t c) {
  printf("0x%02x ", c & 0xFF);
}

// Prints a Byte matrix of a given size
void printMat(uint8_t** mat, uint16_t num_rows, uint16_t num_cols) {
  for (int r = 0; r < num_rows; r++) {
...
uint8_t sBoxLookup(uint8_t byte) {
  return sbox_enc[byte];
}

uint8_t sBoxLookupInv(uint8_t byte) {
  return sbox_dec[byte];
}
...
uint8_t** subBytes(uint8_t** state) {
  ...
  for (int r = 0; r < 4; r++) {
    for (int c = 0; c < 4; c++) {
      new_state[r][c] = sBoxLookup(state[r][c]);
    }
  }
  return new_state;
}
...
uint8_t** addRoundKey(uint8_t** state, uint32_t* w, uint16_t round, uint16_t Nb) {
  ...
  for (int c = 0; c < 4; c++) {
    ...
    new_state[3][c] = state[3][c] ^ a3;
  }
  return new_state;
}

// Returns a word formed from several distinct Byte values
...
uint32_t subWord(uint32_t word) {
  uint32_t ret = 0;
  uint8_t a0 = (word >> 0) & 0xFF;
  uint8_t a1 = (word >> 8) & 0xFF;
  uint8_t a2 = (word >> 16) & 0xFF;
  uint8_t a3 = (word >> 24) & 0xFF;
  ret = buildWord(sBoxLookup(a0), sBoxLookup(a1), sBoxLookup(a2), sBoxLookup(a3));
  return ret;
}
...
uint8_t* encrypt(uint8_t* key, uint8_t* plaintext, uint16_t Nb, uint16_t Nk, uint16_t Nr) {
  ...
  for (uint16_t round = 1; round < Nr; round++) {
...
uint8_t* decrypt(uint8_t* key, uint8_t* ciphertext, uint16_t Nb, uint16_t Nk, uint16_t Nr) {
  ...
  for (uint16_t round = Nr - 1; round > 0; round--) {
...
// Prints a stream of Byte values
...
// Prints matrices of Byte values
...
// Adds values to the buffer in order to encrypt while still communicating the original length
void pad(uint8_t* arr, uint16_t length) {
  if (length > 16) {
    arr[16] = 1;
  }
  else {
    arr[16] = 0;
    for (uint16_t i = length; i < 16; i++) {
      arr[i] = (length & 0xff);
    }
  }
}
